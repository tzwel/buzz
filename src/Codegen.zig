...
fn generateAsyncCall(self: *Self, node: Ast.Node.Index, breaks: ?*std.ArrayList(usize)) Error!?*obj.ObjFunction {
    ...
    const callee = self.ast.nodes.items(.components)[call_node].Call.callee;
    if (type_defs[callee].?.def_type == .Enum) {
        self.reporter.reportErrorAt(
            .fiber_call_not_allowed,
            self.ast.tokens.get(locations[callee]),
        ...
        if (protocol_type_def.def_type == .Placeholder) {
            ...
            while (method_it.next()) |mkv| {
                for (components.members) |member| {
                    if (member.method and std.mem.eql(u8, mkv.key_ptr.*, lexemes[member.name])) {
                        const method_type_def = type_defs[member.method_or_default_value.?].?;
                        if (method_type_def.def_type == .Placeholder) {
                            self.reporter.reportPlaceholder(self.ast, method_type_def.resolved_type.?.Placeholder);
                        } else if (!mkv.value_ptr.*.eql(method_type_def)) {
                            self.reporter.reportTypeCheck(
                                .protocol_conforming,
...
fn generateTry(self: *Self, node: Ast.Node.Index, breaks: ?*std.ArrayList(usize)) Error!?*obj.ObjFunction {
    ...
    for (components.clauses) |clause| {
        ...
        try self.emitOpCode(locations[clause.identifier], .OP_COPY); // Copy error value since its argument to the catch clause
        try self.emitConstant(locations[clause.identifier], error_type.toValue());
        try self.emitOpCode(locations[clause.identifier], .OP_IS);
        ...
        // Pop `is` result
        try self.emitOpCode(locations[clause.identifier], .OP_POP);
...
fn generateTypeExpression(self: *Self, node: Ast.Node.Index, _: ?*std.ArrayList(usize)) Error!?*obj.ObjFunction {
    try self.emitConstant(
        self.ast.nodes.items(.location)[node],
        self.ast.nodes.items(.type_def)[node].?.toValue(),
    );
