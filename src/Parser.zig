...
pub fn advance(self: *Self) !void {
    ...
    if (self.current_token.? >= self.ast.tokens.len) {
        while (true) {
            ...
            const n = try self.ast.appendToken(new_token);
            _ = n;

            // std.debug.print("Token {} is {s}\n", .{ n, @tagName(self.ast.tokens.items(.tag)[n]) });
...
fn declaration(self: *Self) Error!?Ast.Node.Index {
    ...
    if (try self.match(.Extern)) {
        ...
        var node = if (global_scope and !constant and try self.match(.Object))
            ...
            if (global_scope and current_function_type != .Repl) {
...
fn parseGenericResolve(self: *Self, callee_type_def: *obj.ObjTypeDef) Error!Ast.Node.Index {
    const start_location = self.current_token.? - 1;
...
fn genericResolve(self: *Self, _: bool, expr: Ast.Node.Index) Error!Ast.Node.Index {
    return try self.parseGenericResolve(self.ast.nodes.items(.type_def)[expr].?);
}
...
fn function(
    ...
    if (return_type_node) |rtn| {
        self.ast.nodes.items(.type_def)[rtn] = if (self.ast.nodes.items(.type_def)[rtn]) |rt|
            try rt.toInstance(self.gc.allocator, &self.gc.type_registry)
        else
            null;
...
fn asyncCall(self: *Self, _: bool) Error!Ast.Node.Index {
    ...
    const function_type = self.ast.nodes.items(.type_def)[call_components.Call.callee];
...
fn typeExpression(self: *Self, _: bool) Error!Ast.Node.Index {
    ...
    return try self.ast.appendNode(
        .{
            ...
            .location = start_location,
            .end_location = self.current_token.? - 1,
            .type_def = self.ast.nodes.items(.type_def)[type_def],
            .components = .{
